...
QUALITY_MAP = {"360p": 360, "480p": 480, "720p": 720, "1080p": 1080, "4k": 2160}
...
def download_video(
    ...
    output_path = os.path.join(BASE_DOWNLOAD_DIR, "%(id)s.%(ext)s")
    result = []
    ...
    if file_format == "mp3":
        ...
        if quality not in QUALITY_MAP:
            raise RuntimeError(
                f"Invalid quality value: {quality}. Must be 360p, 480p, 720p, 1080p, 4K etc."
            )
    ...
    try:
        with YoutubeDL(ydl_opts) as ydl:
            ...
            try:
                ...
                raise RuntimeError(
                    "Environment variables MAX_DURATION or MAX_SIZE must be integers"
                )
            ...
            for entry in entries:
                ...
                original_file_path = os.path.join(
                    BASE_DOWNLOAD_DIR, f"{original_id}.{extension}"
                )
                original_file_path
                ...
                if start_time and end_time:
                    trimmed_file_path = os.path.join(
                        BASE_DOWNLOAD_DIR, f"{uuid.uuid4()}.{extension}"
                    )
                    try:
                        ffmpeg_cmd = [
                            "ffmpeg",
                ...
                metadata = {
                    ...
                    "published_date": datetime.strptime(
                        entry.get("upload_date", "19700101"), "%Y%m%d"
                    ).date(),
                }
